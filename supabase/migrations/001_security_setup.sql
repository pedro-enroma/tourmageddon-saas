...
-- Policies: authenticated can read, service_role can do everything
CREATE POLICY "app_users_select_auth" ON app_users
  FOR SELECT TO authenticated USING (true);
CREATE POLICY "app_users_all_service" ON app_users
...
-- Policies: authenticated can read, only service_role can insert
...
-- =====================================================
-- PART 3: ENABLE RLS ON ALL EXISTING TABLES
-- =====================================================
...
-- Finance tables
ALTER TABLE IF EXISTS finance_report_data ENABLE ROW LEVEL SECURITY;
...
-- Pattern: authenticated can SELECT, service_role can do ALL
-- =====================================================

-- Helper function to create policies (run policies individually if this fails)
DO $$
DECLARE
  tbl TEXT;
  tables TEXT[] := ARRAY[
    ...
    'participant_sync_logs', 'sync_checkpoints', 'finance_report_data',
...
-- Policies for ticket-vouchers bucket
DO $$
BEGIN
...
-- Policies for service-attachments bucket
DO $$
BEGIN
...
-- =====================================================
-- PART 6: TRIGGER TO UPDATE app_users.updated_at
-- =====================================================
...
-- =====================================================
-- PART 7: CREATE INITIAL ADMIN USER
-- (Run this separately after the user is created in Supabase Auth)
-- =====================================================

-- To create the first admin user after they sign up:
-- INSERT INTO app_users (id, email, full_name, role, is_active)
-- VALUES ('user-uuid-here', 'admin@example.com', 'Admin Name', 'admin', true);
...
-- Check app_users and audit_logs tables exist
