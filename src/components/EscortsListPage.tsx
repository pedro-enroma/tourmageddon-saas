...
export default function EscortsListPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingEscort) {
        // Update existing escort via API
        const result = await escortsApi.update({
          escort_id: editingEscort.escort_id,
        ...
        // Create new escort via API
        const result = await escortsApi.create({
          first_name: formData.first_name,
          last_name: formData.last_name,
          email: formData.email,
          phone_number: formData.phone_number || undefined,
  ...
  const handleDelete = async (escortId: string) => {
    ...
    try {
      // Delete via API
      const result = await escortsApi.delete(escortId)
      if (result.error) throw new Error(result.error)
      fetchEscorts()
    } catch (err) {
