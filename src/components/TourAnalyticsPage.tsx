...
export default function TourAnalyticsPage() {
  ...
  const exportToExcel = () => {
    ...
    // By Date sheet
    const dateSheet = XLSX.utils.json_to_sheet(sanitizeDataForExcel(data.by_date as unknown as Record<string, unknown>[]))
    XLSX.utils.book_append_sheet(wb, dateSheet, 'By Date')

    // By Tour sheet
    const tourSheet = XLSX.utils.json_to_sheet(sanitizeDataForExcel(data.by_tour as unknown as Record<string, unknown>[]))
    XLSX.utils.book_append_sheet(wb, tourSheet, 'By Tour')

    // By Seller sheet
    const sellerSheet = XLSX.utils.json_to_sheet(sanitizeDataForExcel(data.by_seller as unknown as Record<string, unknown>[]))
    XLSX.utils.book_append_sheet(wb, sellerSheet, 'By Seller')

    // By Affiliate sheet
    const affiliateSheet = XLSX.utils.json_to_sheet(sanitizeDataForExcel(data.by_affiliate as unknown as Record<string, unknown>[]))
    XLSX.utils.book_append_sheet(wb, affiliateSheet, 'By Affiliate')

    // By Product sheet
    const productSheet = XLSX.utils.json_to_sheet(sanitizeDataForExcel(data.by_product as unknown as Record<string, unknown>[]))
    XLSX.utils.book_append_sheet(wb, productSheet, 'By Product')

    // By Promo Code sheet
    const promoCodeSheet = XLSX.utils.json_to_sheet(sanitizeDataForExcel(data.by_promo_code as unknown as Record<string, unknown>[]))
