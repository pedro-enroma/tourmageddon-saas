...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const guideAssignments = guideResult.data || []
    const escortAssignments = escortResult.data || []
    const headphoneAssignments = headphoneResult.data || []

    // Collect all unique availability IDs from all assignment types
    const allAvailabilityIds = new Set<number>()
    guideAssignments.forEach(a => allAvailabilityIds.add(a.activity_availability_id))
    escortAssignments.forEach(a => allAvailabilityIds.add(a.activity_availability_id))
    headphoneAssignments.forEach(a => allAvailabilityIds.add(a.activity_availability_id))

    if (allAvailabilityIds.size === 0) {
      return NextResponse.json({ data: [] })
    }

    // Single query for all availabilities
    const { data: availabilities } = await supabase
      .from('activity_availability')
      .select('id, local_date, local_time, vacancy_sold, vacancy_opening, status, activity_id')
      .in('id', Array.from(allAvailabilityIds))
    ...
    const { data: activities } = activityIds.length > 0
      ? await supabase.from('activities').select('activity_id, title').in('activity_id', activityIds)
      : { data: [] }
